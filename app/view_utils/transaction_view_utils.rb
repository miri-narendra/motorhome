  ...
  def payment_split_messages(transaction, conversation, payment, name_display_type)
    if transaction.present? && payment.present? && payment.payment_splits.present?
      ...
      payment.payment_splits.select do |payment_split|
        ...
        date  = payment_split.is_refunded? &&  payment_split.paid_on ? (payment_split.paid_on).to_time : payment_split.updated_at
        create_message_from_action({to_state: 'paid', from_state: 'paid', created_at: date}, 'paid', author, starter, payment_split.sum)
      end
    else
      []
    end
