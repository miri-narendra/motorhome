module TransactionHelper
  ...
  # rubocop:disable all
  def conversation_icon_and_status(status, is_author, other_party_name, waiting_feedback, status_meta, payment_schedule)
    ...
    status = if payment_schedule.waiting_refund?
      "waiting_refund"
    elsif waiting_feedback
      "waiting_feedback"
    else
      "completed"
    ...
    status_hash = {
      ...
      accepted: ->() {
        if !payment_schedule.nil? && payment_schedule.will_have_to_pay_later?
          {
            author: {
              icon: icon_waiting_other,
              text: t("conversations.status.next_payment_from_requester_due_on", requester_name: other_party_name, due_on: l(payment_schedule.next_payment_due_on, format: :default))
            },
            starter: {
              icon: icon_waiting_you,
              text: t("conversations.status.next_payment_from_you_due_on_can_pay_early", due_on: l(payment_schedule.next_payment_due_on, format: :default), pay_now_link: starter_pay_now_link(payment_schedule)).html_safe
            }
          }
        else
          {
            ...
            starter: {
              icon: icon_waiting_you,
              text: t("conversations.status.waiting_payment_from_you", pay_now_link: starter_pay_now_link(payment_schedule)).html_safe
            }
          }
        end
      },
        ...
        if !payment_schedule.nil? && payment_schedule.will_have_to_pay_later?
          {
            ...
            starter: {
              icon: icon_waiting_you,
              text: t("conversations.status.next_payment_from_you_due_on", due_on: l(payment_schedule.next_payment_due_on, format: :default))
            }
          }
        elsif !payment_schedule.nil? && payment_schedule.have_to_pay_now?
          {
            author: {
              icon: icon_waiting_other,
              text: t("conversations.status.waiting_payment_from_requester", requester_name: other_party_name)
      ...
      waiting_refund: ->() { {
        author: {
          icon: icon_waiting_you,
          text: t("conversations.status.waiting_you_to_refund")
        },
        starter: {
          icon: icon_waiting_other,
          text: t("conversations.status.waiting_for_refund", author_name: other_party_name)
        }
      } },
    }
  ...
  #   }
  # }
  def get_conversation_statuses(conversation, is_author, payment_schedule = nil)
    statuses = if conversation.listing && !conversation.status.eql?("free")
      status_hash = {
        ...
        accepted: ->() {
          ...
          statuses = {
            author: [
              ...
              accepted_status(conversation, payment_schedule)
            ]
          }
          statuses[:starter] = if payment_schedule.will_have_to_pay_later?
            [
              status_info(t("conversations.status.due_payments_paid"), icon_classes: icon_for("accepted")),
              pay_early_status(conversation, payment_schedule)
            ]
          else
            [
              status_info(t("conversations.status.request_accepted"), icon_classes: icon_for("accepted")),
        ...
        paid: ->() {
          ...
          #if the rental price has been paid, but the security deposit is still open, then the payment is marked as paid, but
          #the scedule still shows that the transaction still requires a payment,
          payment_schedule = TransactionService::PaymentSchedule.get(community_id: conversation.community_id, transaction_id: conversation.id)
          statuses[:starter] = if payment_schedule.all_payments_paid?
            [
              status_info(t("conversations.status.request_paid"), icon_classes: icon_for("paid")),
              delivery_status(conversation),
              paid_status(conversation, @current_community.testimonials_in_use)
            ]
          elsif payment_schedule.will_have_to_pay_later?
            [
              status_info(t("conversations.status.due_payments_paid"), icon_classes: icon_for("accepted")),
              pay_early_status(conversation, payment_schedule)
            ]
  ...
  def accepted_status(conversation, payment_schedule)
    if conversation.seller == @current_user
      waiting_for_buyer_to_pay(conversation)
    else
      waiting_for_current_user_to_pay(conversation, payment_schedule)
    end
  end

  def pay_early_status(conversation, payment_schedule)
    if conversation.starter == @current_user
      waiting_for_current_user_to_pay_second_payment_early(conversation, payment_schedule)
    end
  end
  ...
  def waiting_for_current_user_to_pay(conversation, payment_schedule)
    pay = {
      link_href: @current_community.payment_gateway.new_payment_path(@current_user, conversation, params[:locale]),
      link_classes: "accept",
    ...
    pay_only_due = {
      ...
      link_classes: "accept",
      link_icon_with_text_classes: 'ss-coins',
      link_text_with_icon: t("conversations.status.pay_only_due")
    }
    pay_upfront = {
      link_href: @current_community.payment_gateway.new_payment_path(@current_user, conversation, params[:locale], {pay_upfront: true}),
      link_classes: "accept",
      link_icon_with_text_classes: 'ss-coins',
      link_text_with_icon: t("conversations.status.pay_upfront")
    }
    cancel = {
    ...
    if payment_schedule.can_pay_upfront?
      if payment_schedule.paid_sum > 0
        status_links([pay_only_due, pay_upfront])
      else
        status_links([pay_only_due, pay_upfront, cancel])
      end
    else
      if payment_schedule.paid_sum > 0
        status_links([pay])
      else
        status_links([pay, cancel])
      end
    end
  end

  def waiting_for_current_user_to_pay_second_payment_early(conversation, payment_schedule)
    pay_second_payment_early = {
      link_href: @current_community.payment_gateway.new_payment_path(@current_user, conversation, params[:locale], {pay_second_payment_early: true}),
      link_classes: "accept",
      link_icon_with_text_classes: 'ss-coins',
      link_text_with_icon: t("conversations.status.pay_second_payment_early")
    }
    if payment_schedule.can_pay_second_payment_early?
      status_links([pay_second_payment_early])
    end
  end
  ...
  def waiting_for_author_acceptance(conversation)
    link = t(
      ...
      :listing_author_name => link_to(
        conversation.author.given_name_or_username,
        conversation.author
      )
    ).html_safe
  ...
  def starter_pay_now_link(payment_schedule)
    link_to person_transaction_path(person_id: payment_schedule.transaction.starter_id, id: payment_schedule.transaction_id), {class: "small-button"} do
      t("conversations.status.pay_now_link_title")
    end
  end

  def refund_status(conversation)
    status_links([
      {
        link_href: full_refund_person_message_path(@current_user, :id => conversation.id),
        link_classes: "accept",
        link_icon_with_text_classes: icon_for("accepted"),
        link_text_with_icon: link_text_with_icon(conversation, "full_refund")
      },
      {
        link_href: partial_refund_person_message_path(@current_user, :id => conversation.id),
        link_classes: "reject",
        icon_classes: icon_for("rejected"),
        link_text_with_icon: link_text_with_icon(conversation, "partial_refund")
      }
    ])
  end
end
