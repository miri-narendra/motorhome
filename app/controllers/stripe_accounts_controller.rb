class StripeAccountsController < ApplicationController
  ...
  before_filter do |controller|
	...
	@adminaccount_path = new_stripe_settings_payment_path(@current_user)
	...
	@stripe_connect_admin_uri = StripeOauthService.connect_merchant_request_url(@adminaccount_path, @current_community)
  end

  # New/create
  before_filter :ensure_user_does_not_have_connected_account, :only => [:new, :create, :adminaccount]
  ...
  def new
    ...
    render locals: { stripe_connect_url: @stripe_connect_uri, stripe_connect_admin_url: @stripe_connect_admin_uri }
  end
