class AcceptConversationsController < ApplicationController
  ...
  def prepare_accept_or_reject_form
    ...
    #cart must be built before payment is done, because payment relies on cart to get security_deposit
    transaction = Transaction.find(transaction_hash[:id])
    TransactionService::Cart.get_or_create(transaction: transaction_hash)
