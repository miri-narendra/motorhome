class PeopleController < Devise::RegistrationsController
  ...
  def show
    ...
    include_closed = ( @current_user == @person || (@current_user.present? && @current_user.is_admin?) ) && params[:show_closed]
    search = {
      author_id: @person.id,
  ...
  def about_gossrv
    @person = Person.find("mgb54-MiUv3UFp_mYMvtOg")
  end
  ...
  def new
    ...
    @person = if params[:person] then
      Person.new(params[:person].slice(:given_name, :family_name, :email, :username, :is_owner, :is_owner_extra))
    else
      Person.new()
    end
  ...
  def create
    ...
    if @current_community
      membership = CommunityMembership.new(:person => @person, :is_owner => params[:is_owner], :is_owner_extra => params[:is_owner_extra], :community => @current_community, :consent => @current_community.consent)
    ...
    Delayed::Job.enqueue(CreateSalesforceLeadJob.new(@person.id, @current_community.id)) if @current_community
  ...
  def check_phone_number_validity
    respond_to do |format|
      format.json { render json: !!(params[:person][:phone_number] =~ PHONE_NUMBER_FORMAT) }
    end
  end
